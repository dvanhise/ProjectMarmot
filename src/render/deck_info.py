...
def render_deck_info(s: pygame.Surface, player: Player):
    ...
    text_rect = text.get_rect(center=(INFO_CARD_SIZE[0]//2, INFO_CARD_SIZE[1]*2//4))
    card_surface.blit(text, text_rect)

    text = font.render(f'Deleted: {len(player.deleted_pile)}', True, 'white')
    text_rect = text.get_rect(center=(INFO_CARD_SIZE[0]//2, INFO_CARD_SIZE[1]*3//4))
    card_surface.blit(text, text_rect)

    s.blit(card_surface, (SCREEN_WIDTH-200, SCREEN_HEIGHT-CARD_HEIGHT))
